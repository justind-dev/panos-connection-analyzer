...
class Reporter:
    ...
    def _add_nat_summary_sheet(self, workbook, df: pd.DataFrame):
        ...
        if 'nat_rule' in df.columns and df['nat_rule'].notna().any():
            ...
            nat_rules = df[df['nat_rule'].notna() & (df['nat_rule'] != '')].groupby('nat_rule').size().reset_index(name='count')
        ...
        if 'nat_type' in df.columns and df['nat_type'].notna().any():
            ...
            nat_types = df[df['nat_type'].notna() & (df['nat_type'] != '')].groupby('nat_type').size().reset_index(name='count')
    ...
    def _print_summary(self, df: pd.DataFrame):
        ...
        if any(field in df.columns for field in nat_fields):
            ...
            nat_ip_count = df[(df[nat_fields[0]].notna() | df[nat_fields[1]].notna()) if nat_fields[0] in df.columns and nat_fields[1] in df.columns else False].shape[0]
            if nat_ip_count > 0:
            ...
            if 'nat_rule' in df.columns and df['nat_rule'].notna().any():
                ...
                print(f"Connections with identified NAT rules: {nat_rule_count} ({nat_rule_count/len(df)*100:.1f}% of total)")
        ...
        if any(field in df.columns for field in addr_fields):
            ...
            if addr_count > 0:
                ...
                print(f"Connections with identified address objects: {addr_count} ({addr_count/len(df)*100:.1f}% of total)")
